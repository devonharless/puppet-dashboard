...
var text = RGraph.parseTooltipText(obj.Get('chart.tooltips'), idx);if(text.match(/^id:(.*)$/) && RGraph.getTooltipTextFromDIV(text).substring(0,3) == 'id:'){
return;}
if(   mouseX > (xCoord - 5)
&& mouseX < (xCoord + 5)
&& mouseY > (yCoord - 5)
...
RGraph.Tooltip(canvas, text, e.pageX, obj.Get('chart.tooltips.hotspot.xonly') ? (point[2] + RGraph.getCanvasXY(canvas)[1]) : e.pageY, idx);RGraph.Registry.Get('chart.tooltip').__index__ = Number(idx);while (idx >= obj.data[0].length){
